...
$featureConfig = @{
    ...
    "F-063" = @{ RustFile=$null; TsFiles=@("src\screens\AnalyticsDashboardScreen.tsx") }
    "F-064" = @{ RustFile="src-tauri\src\commands\maintenance.rs"; ModFile="src-tauri\src\commands\mod.rs"; ModLine="pub mod maintenance;"; Commands=@("commands::maintenance::run_maintenance_check","commands::maintenance::get_maintenance_history","commands::maintenance::apply_maintenance_patch"); SqlFile="src-tauri\migrations\014_maintenance.sql"; SqlVersion=14; SqlDesc="create_maintenance"; TsFiles=@("src\screens\MaintenanceScreen.tsx"); ExtraCrates=@('rusqlite = { version = "0.31", features = ["bundled"] }') }
...
$prompts["F-063"] = "CRITICAL TYPESCRIPT RULES - follow exactly:
...
- Return type inference is fine, no need to annotate every function`n`nI'm building LINUP.IO - Tauri v2 + React/TypeScript Vite desktop app.`nCONTEXT: File src/screens/AnalyticsDashboardScreen.tsx. Shows PostHog analytics for a generated app inside LINUP. Uses PostHog REST API (api.posthog.com/api/projects/{project_id}/insights). PostHog API key stored in keyring via F-060 secrets system.`nTASK: Output ONE tsx block for AnalyticsDashboardScreen. Props: {projectId:string}. Sections: (1) Key metrics row - pageviews today, unique visitors, top page, conversion rate - each as a stat card fetched from PostHog API via invoke get_posthog_stats. (2) Pageviews chart - 7-day line chart using recharts LineChart. (3) Top pages table - path, views, unique visitors, avg time. (4) Events feed - last 10 custom events from mec.track() calls. Empty state when no PostHog key configured with link to secrets wizard. use import type for all type imports. Output ONLY tsx block. No prose."

$prompts["F-064"] = "I'm building LINUP.IO - Tauri v2 Rust + React/TypeScript.`n`n$rustDb`n`nCONTEXT: Maintenance mode. Runs quarterly dependency audit outside the main pipeline. Reads project folder_path from DB, checks npm outdated and npm audit, calls Anthropic haiku to generate a minimal patch updating vulnerable or outdated packages, runs test suite, presents as mini approval gate using existing EvidencePackViewer component.`n`nTASK: Output THREE blocks.`n1) sql: CREATE TABLE maintenance_runs(id TEXT PRIMARY KEY,project_id TEXT NOT NULL,run_type TEXT NOT NULL DEFAULT dependency_update,status TEXT NOT NULL DEFAULT pending,packages_checked INTEGER,vulnerabilities_found INTEGER,packages_updated INTEGER,artifact_id TEXT,created_at TEXT NOT NULL,completed_at TEXT).`n2) rust async with open_db() +:`n   - MaintenanceResult{id:String,packages_checked:i64,vulnerabilities_found:i64,packages_updated:i64,artifact_id:String,status:String}(Serialize+Deserialize)`n   - run_maintenance_check(app:tauri::AppHandle,project_id:String)->Result<MaintenanceResult,String> async: get folder_path, run npm audit --json parse vulnerabilities, run pnpm outdated --json parse outdated list, call Anthropic haiku to generate minimal package.json update patch, write patch artifact, run pnpm install + test suite, emit maintenance:complete`n   - get_maintenance_history(project_id:String)->Result<Vec<MaintenanceRun>,String>`n   - apply_maintenance_patch(project_id:String,maintenance_id:String)->Result<(),String> applies stored patch and triggers redeploy`n3) tsx for src/screens/MaintenanceScreen.tsx: shows maintenance history list, Run check now button, for pending patches shows mini evidence pack with what packages updated+why+test results+Approve to apply, uses EvidencePackViewer component. use import type for all type imports.`n`nOutput ONLY three blocks. No prose."

$prompts["F-090"] = "I'm building LINUP.IO - Tauri v2 Rust + React/TypeScript.`n`n$rustDb`n`n$mecContext`n`nCONTEXT: File src-tauri/src/commands/mec.rs. DB E:\\linup-io\\linup.db. LINUP platform MEC command layer - tracks 1.5% revenue share for the LINUP control plane. Zero if app earns zero.`n`nTASK: Output TWO blocks. 1) sql: CREATE TABLE mec_revenue_events(id TEXT PRIMARY KEY,project_id TEXT NOT NULL,app_id TEXT NOT NULL,gross_amount_pence INTEGER NOT NULL,mec_share_pence INTEGER NOT NULL,currency TEXT NOT NULL DEFAULT gbp,stripe_payment_intent_id TEXT,status TEXT NOT NULL DEFAULT pending,created_at TEXT NOT NULL); CREATE TABLE mec_entitlement_log(id TEXT PRIMARY KEY,project_id TEXT NOT NULL,feature_key TEXT NOT NULL,result TEXT NOT NULL,plan TEXT NOT NULL,checked_at TEXT NOT NULL). 2) rust: open_db() + report_revenue(project_id:String,app_id:String,gross_amount_pence:i64,currency:String,stripe_payment_intent_id:Option<String>)->Result<String,String> mec_share_pence=(gross_amount_pence*15/1000) INSERT mec_revenue_events return id, check_entitlement(project_id:String,feature_key:String,plan:String)->Result<bool,String> logs to mec_entitlement_log, get_mec_summary(project_id:String)->Result<MecSummary,String> MecSummary{total_gross_pence:i64,total_share_pence:i64,share_rate:f64,event_count:i64}(Serialize+Deserialize), create_experiment(project_id:String,name:String,variants:Vec<String>,metric:String)->Result<String,String> inserts into experiments table returns experiment_id, get_experiment_results(project_id:String,experiment_id:String)->Result<ExperimentResults,String> ExperimentResults{experiment_id:String,name:String,variants:Vec<VariantResult>} VariantResult{name:String,visitors:i64,conversions:i64,revenue_pence:i64,conversion_rate:f64}. ALSO add to sql block: CREATE TABLE mec_experiments(id TEXT PRIMARY KEY,project_id TEXT NOT NULL,name TEXT NOT NULL,variants TEXT NOT NULL,metric TEXT NOT NULL,status TEXT NOT NULL DEFAULT running,created_at TEXT NOT NULL,concluded_at TEXT); CREATE TABLE mec_experiment_events(id TEXT PRIMARY KEY,experiment_id TEXT NOT NULL,variant TEXT NOT NULL,event_type TEXT NOT NULL,revenue_pence INTEGER DEFAULT 0,created_at TEXT NOT NULL). Output ONLY two blocks. No prose."
