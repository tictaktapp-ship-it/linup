...
$rustDb = "CRITICAL RUST DB PATTERN - follow exactly:`n- Use rusqlite crate directly (NOT tauri-plugin-sql, NOT sqlx, NOT DbPool, NOT State)`n- tauri-plugin-sql is JavaScript-only. Cannot be used from Rust commands.`n- Open DB pattern:`n`nuse rusqlite::{Connection, params};`nconst DB_PATH: &str = `"

$mecContext = "MEC (Monetization and Entitlement Core) RULES - bake into every generated app:`n- Every scaffold (F-015) must inject MEC SDK into the generated app`n- MEC SDK package: @linup/mec-sdk`n- Three calls available in generated app: mec.can(featureKey), mec.track(eventName, metadata), mec.billing.reportRevenue(amount, currency, stripePaymentIntentId)`n- Two tables injected into generated app migrations: revenue_events and entitlement_checks`n- LINUP takes 1.5% of gross revenue via Stripe Connect - zero if app earns zero`n- MEC must be present for Stage 8 hardening gates to pass`n- Disclosure required in generated app README and onboarding Step 4"E:\\\\linup-io\\\\linup.db`";`nfn open_db() -> Result<Connection, String> { Connection::open(DB_PATH).map_err(|e| format!(`"DB error: {e}`")) }`n`n- Commands are synchronous (pub fn) unless calling reqwest (then pub async fn)`n- Use params![] for all query parameters`n- UUIDs: format!(`"{}`", uuid::Uuid::new_v4())`n- All structs: #[derive(Debug, Serialize, Deserialize)]`n- Do NOT use sqlx, DbPool, or State for DB access"
...
$featureConfig = @{
    ...
    "F-090" = @{ RustFile="src-tauri\src\commands\mec.rs"; ModFile="src-tauri\src\commands\mod.rs"; ModLine="pub mod mec;"; Commands=@("commands::mec::report_revenue","commands::mec::check_entitlement","commands::mec::get_mec_summary"); SqlFile="src-tauri\migrations\012_mec.sql"; SqlVersion=12; SqlDesc="create_mec_tables"; TsFiles=@("src\screens\MecSetupScreen.tsx"); ExtraCrates=@('rusqlite = { version = "0.31", features = ["bundled"] }') }'); TsFiles=@("src\screens\UpdatesScreen.tsx") }
}
...
$prompts["F-040"] = "I'm building LINUP.IO - Tauri v2 Rust + React/TypeScript.`n`n$rustDb`n`nCONTEXT: File src-tauri/src/commands/budget.rs. DB E:\\linup-io\\linup.db. stage_runs owned by F-003 do NOT recreate. projects has budget_cap REAL. Emit budget:exceeded via app.emit().`n`nTASK: Output TWO blocks. 1) sql: ALTER TABLE stage_runs ADD COLUMN IF NOT EXISTS cost_tokens_in INTEGER NOT NULL DEFAULT 0; ALTER TABLE stage_runs ADD COLUMN IF NOT EXISTS cost_tokens_out INTEGER NOT NULL DEFAULT 0; 2) rust: open_db(), BudgetStatus{spent:f64,cap:f64,state:String}(Serialize+Deserialize), check_budget(app:tauri::AppHandle,project_id:String)->Result<BudgetStatus,String> sum cost_usd compare to budget_cap states Safe<80 Warning>=80 Critical>=95 Exceeded>=100 if Exceeded app.emit(budget:exceeded).ok(), raise_cap(project_id:String,new_cap:f64)->Result<(),String> validate new_cap>spent. Output ONLY two blocks. No prose."
...
$prompts["F-014"] = "I'm building LINUP.IO - Tauri v2 Rust + React/TypeScript.`n`n$rustDb`n`nCONTEXT: File src-tauri/src/commands/pipeline/stage5_architecture.rs. DB E:\\linup-io\\linup.db. Same Anthropic async pattern as F-013. Stage index 4. Load approved product_spec artifact write architecture artifact.`n`nTASK: Output ONE rust block for run_stage_architecture(app:tauri::AppHandle,project_id:String)->Result<StageResult,String>. System prompt: architecture doc with data model API surfaces auth boundaries threat model deployment plan. Gate check data model+auth+deployment. Update stage awaiting_approval. Output ONLY rust block. No prose."
$prompts["F-015"] = "I'm building LINUP.IO - Tauri v2 Rust + React/TypeScript.`n`n$rustDb`n`nCONTEXT: File src-tauri/src/commands/pipeline/stage6_scaffold.rs. DB E:\\linup-io\\linup.db. Creates files in user project folder from projects.folder_path. SAFE shell-out std::process::Command Vec<String> args ONLY. Next.js+Supabase stack.`n`nTASK: Output ONE rust block with: open_db(), scaffold_project(app:tauri::AppHandle,project_id:String)->Result<ScaffoldResult,String> ScaffoldResult:{files_created:Vec<String>,artifact_id:String} create package.json+src/app/page.tsx+src/lib/supabase.ts+.env.example+supabase/migrations/.gitkeep+.gitignore write scaffold_summary artifact update stage awaiting_approval, get_scaffold_status(project_id)->Result<String,String>, open_project_folder(project_id)->Result<(),String> using explorer. CRITICAL: The scaffold must also create these two migration files in the generated app: (1) CREATE TABLE revenue_events(id TEXT PRIMARY KEY, app_id TEXT NOT NULL, event_type TEXT NOT NULL, gross_amount INTEGER NOT NULL, currency TEXT NOT NULL DEFAULT gbp, stripe_payment_intent_id TEXT, mec_share_amount INTEGER NOT NULL, created_at TEXT NOT NULL); (2) CREATE TABLE entitlement_checks(id TEXT PRIMARY KEY, app_id TEXT NOT NULL, feature_key TEXT NOT NULL, result TEXT NOT NULL, resolved_at TEXT NOT NULL, plan_snapshot TEXT). Also add to package.json dependencies: "@linup/mec-sdk": "latest". Also add to the generated app root layout: import { MecProvider } from "@linup/mec-sdk"; wrap root with <MecProvider appId={process.env.LINUP_APP_ID} shareRate={0.015}>. Output ONLY rust block. No prose."
